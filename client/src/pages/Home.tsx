...
export default function Home() {
  ...
  return (
    <div id="top" className="site-shell">
      <header className="site-header"><div className="nav-wrap"><Logo /><nav className={menuOpen ? "nav-links nav-links--open" : "nav-links"} aria-label="Primary navigation"><a href="#product" onClick={() => setMenuOpen(false)}>Product</a><a href="#workflow" onClick={() => setMenuOpen(false)}>Solutions</a><a href="#demo" onClick={() => setMenuOpen(false)}>Resources</a><a href="#footer" onClick={() => setMenuOpen(false)}>Pricing</a><a className="mobile-signin" href="#footer" onClick={() => setMenuOpen(false)}>Sign in</a><a className="mobile-cta" href="#start" onClick={() => setMenuOpen(false)}>Start for free <ArrowUpRight size={15} /></a></nav><div className="nav-actions"><a className="signin" href="#footer">Sign in</a><a className="nav-cta" href="#start">Start for free <ArrowUpRight size={15} /></a><button className="menu-button" onClick={() => setMenuOpen(!menuOpen)} aria-label={menuOpen ? "Close menu" : "Open menu"} aria-expanded={menuOpen}>{menuOpen ? <X size={20} /> : <Menu size={20} />}</button></div></div></header>

      <main>
        <section className="hero section-pad"><div className="hero-text"><span className="eyebrow hero-eyebrow"><span className="eyebrow-line" /> Feedback intelligence for product teams</span><h1>Turn customer noise into your <span>next best product decision.</span></h1><p className="hero-copy">SignalFlow brings feedback, support conversations, and product requests into one clear view—so your team can see the pattern and decide what to build next.</p><div className="hero-actions"><a className="button button--primary" href="#start">Start for free <ArrowUpRight size={17} /></a><a className="button button--quiet" href="#product">See how it works <MoveRight size={17} /></a></div><p className="hero-note"><Check size={14} /> No credit card required <span /> <Check size={14} /> Sample workspace included</p></div><div className="hero-art"><img src="/manus-storage/signalflow-hero-texture_82df9ca9.png" alt="Abstract signal lines on a warm paper field" /><div className="hero-art-label"><span>01</span><span>Signal / clarity</span></div><div className="hero-note-card"><span className="pulse-dot" /> <span>New pattern detected</span><strong>Checkout friction</strong><small>38 signals · rising this week</small></div></div></section>

        <section id="product" className="showcase section-pad"><div className="section-intro"><div><span className="eyebrow"><span className="eyebrow-line" /> The product</span><h2>One workspace for the conversations that shape your roadmap.</h2></div><p>SignalFlow turns scattered customer language into a living, inspectable view of what deserves your team’s attention.</p></div><Dashboard /><div className="showcase-caption"><span>Sample workspace / Demo data</span><span>SignalFlow overview <ArrowUpRight size={14} /></span></div></section>

        <section id="workflow" className="workflow-section section-pad"><div className="workflow-image"><img src="/manus-storage/signalflow-insight-field_b5f9e7bc.png" alt="Abstract connected signal traces on a warm grey background" /><div className="image-stamp">FROM NOISE<br /><span>TO SIGNAL</span></div></div><div className="workflow-copy"><span className="eyebrow"><span className="eyebrow-line" /> How it works</span><h2>From noise to signal.</h2><p className="lead-copy">A simple loop for making product decisions with more context and less ceremony.</p><div className="workflow-list">{workflow.map(({ index, title, copy, icon: Icon }) => <div className="workflow-item" key={title}><span className="workflow-index">{index}</span><div className="workflow-icon"><Icon size={17} /></div><div><h3>{title}</h3><p>{copy}</p></div></div>)}</div></div></section>
